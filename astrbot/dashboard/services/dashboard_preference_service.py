...
def _optimize_wallpaper_image(content: bytes, ext: str) -> tuple[bytes, str]:
    """用 Pillow 压缩壁纸图片，返回 (新内容, 新扩展名)。

    规则：最长边超过 WALLPAPER_MAX_EDGE 等比缩到该值内；无透明通道转
    ...
    # 等比缩放：最长边不超过 WALLPAPER_MAX_EDGE
    max_edge = max(img.size)
    if max_edge > WALLPAPER_MAX_EDGE:
        ratio = WALLPAPER_MAX_EDGE / max_edge
        new_size = (
...
class DashboardPreferenceService:
    ...
    def normalize_wallpaper(cls, value: object) -> dict[str, object] | None:
        ...
        if (
            ...
            and opacity == WALLPAPER_OPACITY_MAX
            and panel_opacity == PANEL_OPACITY_MIN
        ):
            # 无壁纸 + 全默认透明度 = 未设置
            return None
        ...
        if mode != WALLPAPER_MODE_SEPARATE:
            result["mode"] = mode
        if opacity != WALLPAPER_OPACITY_MAX:
            result["opacity"] = opacity
        if panel_opacity != PANEL_OPACITY_MIN:
