...
class PluginManager:
    ...
    def _validate_astrbot_version_specifier(
        ...
        try:
            ...
            return (
                False,
                f"Invalid current ldm version: {VERSION}. Cannot check plugin version range.",
            )

        if not specifier.contains(current_version, prereleases=True):
            return (
                False,
                f"ldm {VERSION} does not satisfy plugin astrbot_version: {normalized_spec}",
            )
        return True, None
        ...
        for plugin_module in plugin_modules:
            try:
                    ...
                    if not ignore_version_check:
                        ...
                        if not is_valid:
                            raise PluginVersionUnsupportedError(
                                error_message
                                or "The plugin does not support the current ldm version."
                            )
                ...
                for handler in handlers:
                    try:
                        logger.info(
                            f"钩子(on_plugin_loaded) -> {star_map[handler.handler_module_path].name} - {handler.handler_name}",
                        )
    ...
    async def _terminate_plugin(star_metadata: StarMetadata) -> None:
        ...
        for handler in handlers:
            try:
                logger.info(
                    f"钩子(on_plugin_unloaded) -> {star_map[handler.handler_module_path].name} - {handler.handler_name}",
                )
