...
class AstrBotCoreLifecycle:
    ...
    async def _init_or_reload_subagent_orchestrator(self) -> None:
        ...
        try:
            ...
            logger.error(f"子代理编排器初始化失败: {e}", exc_info=True)
    ...
    def _warn_about_unset_default_chat_provider(self) -> None:
        ...
        if not default_id:
            ...
            logger.warning(
                "检测到已启用 %d 个对话模型，但 `provider_settings.default_provider_id` 为空。"
                "ldm 将使用 `%s` 作为启动时的回退对话模型。"
                "请在 WebUI 配置页设置默认对话模型，以避免意外切换。",
                len(providers),
                fallback_id,
            )
            return
        ...
        if not found:
            self._default_chat_provider_warning_emitted = True
            logger.warning(
                "配置的 `default_provider_id` 为 `%s`，但没有匹配的已启用模型。"
                "ldm 将使用 `%s` 作为回退对话模型。"
                "请检查 WebUI 配置页。",
                default_id,
                fallback_id,
    ...
    async def start(self) -> None:
        ...
        for handler in handlers:
            try:
                logger.info(
                    f"钩子(on_astrbot_loaded) -> {star_map[handler.handler_module_path].name} - {handler.handler_name}",
                )
