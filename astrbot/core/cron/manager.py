...
class CronJobManager:
    ...
    async def _run_job(
        ...
        *,
        ...
        try:
            ...
            status = "failed"
            last_error = str(e)
            logger.error(f"定时任务 {job_id} 失败: {e!s}", exc_info=True)
        finally:
    ...
    async def _woke_main_agent(
        ...
        *,
        ...
        if not result:
            logger.error("为定时任务构建主 Agent 失败。")
            return
        ...
        if not llm_resp:
            logger.warning("定时任务 Agent 未返回响应")
            return
