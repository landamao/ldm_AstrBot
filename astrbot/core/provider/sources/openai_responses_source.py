...
@register_provider_adapter(
    "openai_responses",
    "OpenAI 兼容 Responses API 提供商适配器",
)
class ProviderOpenAIResponses(ProviderOpenAIOfficial):
    """OpenAI 兼容、无服务端会话状态的 Responses API 提供商适配器。"""
    ...
    async def _query(
        ...
        *,
        ...
        response = await retry_provider_request(
            f"OpenAI Responses/{self.display_provider_id()}/{payloads.get('model', self.get_model())}",
        ...
        logger.debug("Responses 返回: %s", response)
