...
@register_provider_adapter(
    ...
    "胜算云 Chat Completion 提供商适配器",
)
class ProviderSSYCloud(ProviderOpenAIOfficial):
    """胜算云提供商，走 OpenAI 兼容 Chat Completions。"""
    ...
    def __init__(self, provider_config: dict, provider_settings: dict) -> None:
        ...
        custom_headers.setdefault("X-Title", "ldmbot")
    ...
    async def get_models(self) -> list[str]:
        ...
        try:
            response = await retry_provider_request(
                f"胜算云/{self.display_provider_id()}",
            ...
            raise Exception(f"获取胜算云模型列表失败: {exc}") from exc
