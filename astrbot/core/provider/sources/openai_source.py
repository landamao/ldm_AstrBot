...
class ProviderOpenAIOfficial(Provider):
    ...
    def _force_stream_on_query_enabled(self) -> bool:
        """是否在非流式查询时底层强制使用 stream=True。

        某些上游/代理只在 stream=True 时返回内容，stream=False 会回空响应。
        开启此开关后，非流式调用会在底层用 stream=True 发请求，再把流聚合成
        完整 ChatCompletion 返回给上层，上层调用方无感知。
        """
        value = self.provider_config.get("force_stream_on_query", False)
        if isinstance(value, str):
            return value.strip().lower() in {"1", "true", "yes", "on"}
        return bool(value)
    ...
    async def _query(
        ...
        *,
        ...
        # 部分上游/代理只在 stream=True 时返回内容，stream=False 会返回空响应
        # （典型表现：Content-Type=text/event-stream，choices 为空，SDK 无法
        # 解析非流式响应体而原样返回一个 str）。
        # 处理策略：
        #   1. 若 force_stream_on_query 开启，直接用 stream=True 发请求；
        #   2. 否则先按 stream=False 发，一旦检测到返回的不是合法 ChatCompletion
        #      （AsyncStream 或 str 等），自动降级为 stream=True 重发一次并聚合。
        # 对正常上游零影响（stream=False 正常返回，不触发降级）。
        force_stream = self._force_stream_on_query_enabled()
        ...
        completion = await _create(stream=True if force_stream else False)

        # 兼容：上游无视 stream=False 强制返回了流式响应。
        # 此时 completion 是 AsyncStream 而非 ChatCompletion，
        # 先把整个流接收完聚合成完整 completion，再按非流式正常返回。
        if isinstance(completion, AsyncStream):
            if not force_stream:
                logger.warning(
                    "上游无视 stream=False 强制返回了流式响应，已自动兼容：流式接收中……",
                )
            completion = await self._collect_forced_stream(completion)

        # 自动降级：非流式请求拿到的不是合法 ChatCompletion（多为原始 str SSE，
        # 说明上游只在 stream=True 时才吐内容）。改用 stream=True 重发并聚合。
        if not force_stream and not isinstance(completion, ChatCompletion):
            logger.warning(
                "非流式请求返回了非法响应（type=%s），疑似上游只支持流式，"
                "已自动降级为 stream=True 重试……",
                type(completion).__name__,
            )
            completion = await _create(stream=True)
            if isinstance(completion, AsyncStream):
                completion = await self._collect_forced_stream(completion)
    ...
    async def _collect_forced_stream(
        ...
        """把被上游强制返回的流式响应完整接收并聚合成 ChatCompletion。

        用于非流式调用（stream=False）却收到流式响应的兼容场景：
        逐块接收完毕后，用 ChatCompletionStreamState 还原出完整 completion，
        使调用方无感知地拿到与非流式一致的结果。
        """
        ...
        async for chunk in stream:
            ...
            # 与 _query_stream 一致：修正部分代理缺失的 tool_call index 字段，
            # 并跳过 delta=None 的空 chunk（流末尾 usage chunk 仍需处理）。
            ...
            if delta is not None or chunk.usage:
                try:
                    ...
                    logger.error("流式接收聚合 chunk 出错: " + str(e))
        logger.debug(f"强制流式响应接收完成，共聚合 {chunk_count} 个 chunk，开始发送。")
    ...
    async def _parse_openai_completion(
        ...
        # 兼容部分网关把真实 completion 嵌在 data 里的情况（#9374）
        if not completion.choices:
