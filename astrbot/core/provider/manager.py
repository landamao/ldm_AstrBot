...
class ProviderManager:
    ...
    async def initialize(self) -> None:
        ...
        async def _init_mcp_clients_bg() -> None:
            try:
                ...
                logger.error("MCP 初始化后台任务失败", exc_info=True)
    ...
    async def load_provider(self, provider_config: dict) -> None:
        ...
        if not provider_config["enable"]:
            logger.info(f"提供商 {provider_config['id']} 已禁用，跳过加载")
            return
        ...
        if provider_config["type"] not in provider_cls_map:
            logger.error(
                f"未找到提供商适配器: {provider_config['type']}({provider_config['id']})，已跳过。",
                exc_info=True,
            )
            return
            ...
            match provider_metadata.provider_type:
                case ProviderType.SPEECH_TO_TEXT:
                    ...
                    if (
                        ...
                        logger.info(
                            f"已选择 {provider_config['type']}({provider_config['id']}) 作为默认语音转文本提供商",
                        )
                ...
                case ProviderType.TEXT_TO_SPEECH:
                    ...
                    if (
                        ...
                        logger.info(
                            f"已选择 {provider_config['type']}({provider_config['id']}) 作为默认文本转语音提供商",
                        )
    ...
    async def reload(self, provider_config: dict) -> None:
        async with self.reload_lock:
            ...
            logger.info(f"用户配置中的提供商: {config_ids}")
    ...
    async def terminate(self) -> None:
        ...
        try:
            ...
            logger.error("禁用 MCP 服务时发生错误", exc_info=True)
