...
class FunctionToolManager:
    ...
    async def init_mcp_clients(
        ...
        for (name, cfg, _), result in zip(active_configs, results, strict=False):
            if isinstance(result, Exception):
                if isinstance(result, MCPInitTimeoutError):
                    logger.error(
                        f"连接 MCP 服务 {name} 超时（{timeout_display} 秒）"
                    )
                else:
                    logger.error(f"初始化 MCP 服务 {name} 失败: {result}")
        ...
        if failed_services:
            logger.warning(
                ...
                f"请检查 mcp_server.json 文件以及服务可用性。"
            )
        ...
        logger.info(
            f"MCP 服务初始化完成: 成功 {summary.success}/{summary.total}，失败 {len(summary.failed)}。"
        )
        if summary.total > 0 and summary.success == 0:
            msg = "所有 MCP 服务均初始化失败，请检查 mcp_server.json 以及服务可用性。"
        ...
        *,
        ...
        async with self._runtime_lock:
            if name in self._mcp_server_runtime or name in self._mcp_starting:
                logger.warning(
                    f"MCP 服务 {name} 已连接，忽略本次启动请求（timeout={timeout:g}）。"
                )
                self._log_safe_mcp_debug_config(cfg)
                return
        ...
        async def connect_and_lifecycle() -> None:
            ...
            logger.info(
                f"已连接 MCP 服务 {name}，"
                f"工具: {[t.name for t in mcp_client.tools]}"
            )
            ...
            try:
                ...
                logger.info(f"已收到 MCP 客户端 {name} 的关闭信号")
            except asyncio.CancelledError:
    ...
    async def _cleanup_mcp_client_safely(
        ...
        try:
            ...
            logger.error(
                f"清理 MCP 客户端资源失败 {name}: {cleanup_exc}"
            )
