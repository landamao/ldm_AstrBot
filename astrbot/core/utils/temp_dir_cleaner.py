...
class TempDirCleaner:
    ...
    def _limit_bytes(self) -> int:
        ...
        if parsed <= 0:
            ...
            logger.warning(
                f"{self.CONFIG_KEY}={configured!r} 无效，回退为 {self.DEFAULT_MAX_SIZE}MB。",
            )
            return fallback
        return parsed
    ...
    def cleanup_once(self) -> None:
        ...
        for file_info in sorted(files, key=lambda item: item.mtime):
            try:
                ...
                logger.warning(f"删除临时文件失败 {file_info.path}: {e}")
                continue
        ...
        logger.warning(
            f"临时目录超出限制（{total_size} > {limit}）。"
            f"已删除 {removed_files} 个文件，释放 {released} 字节"
            f"（目标 {target_release} 字节）。",
        )

    async def run(self) -> None:
        logger.info(
            f"临时目录清理器已启动。间隔={self.CHECK_INTERVAL_SECONDS}s "
            f"清理比例={self.CLEANUP_RATIO}",
        )
        while not self._stop_event.is_set():
            try:
                ...
                logger.error(f"临时目录清理器运行失败: {e}", exc_info=True)
