...
def _migra_agent_runner_configs(conf: AstrBotConfig, ids_map: dict) -> None:
    ...
    try:
        ...
        logger.error(f"第三方 Agent Runner 配置迁移失败: {e!s}")
...
def _migra_provider_to_source_structure(conf: AstrBotConfig) -> None:
    ...
    for provider in providers:
        ...
        migrated = True
        logger.info(f"正在将提供商 {provider.get('id')} 迁移到新结构")
...
async def migra(
    ...
    try:
        ...
        logger.error(f"从 4.5 迁移到 4.6 失败: {e!s}")
        logger.error(traceback.format_exc())
    ...
    try:
        ...
        logger.error(f"webchat 会话迁移失败: {e!s}")
        logger.error(traceback.format_exc())
    ...
    try:
        ...
        logger.error(f"token_usage 字段迁移失败: {e!s}")
        logger.error(traceback.format_exc())
    ...
    try:
        ...
        logger.error(f"提供商-源结构迁移失败: {e!s}")
        logger.error(traceback.format_exc())
