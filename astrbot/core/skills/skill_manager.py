...
def build_skills_prompt(skills: list[SkillInfo]) -> str:
    ...
    return (
        "## Skills\n\n"
        "Skills are reusable instruction bundles stored in `SKILL.md` files. "
        "Each skill has a **name** and a **description**.\n\n"
        "### Available skills\n\n"
        f"{skills_block}\n\n"
        "### Skill loading\n\n"
        "1. The list above is the complete skill inventory for this session. "
        "Full instructions are in the referenced `SKILL.md` file.\n"
        "2. Full skill content is loaded by reading `SKILL.md` via a shell "
        "command compatible with the current runtime shell, using the "
        f"**absolute path** shown above (e.g. `{example_command}`).\n"
        "3. Load only what is directly referenced from `SKILL.md`. "
        "If `scripts/` exist, they can be run directly; if `assets/` or "
        "templates exist, they can be reused.\n"
    )
