...
基于 ldm v4.26.20 的上下文与打断落库修复。

### 修复

- **上下文重复「[昵称(ID)]: 正文」**
  - 历史里有时同时存在 `[懒大猫(204676209)]: 试试发杂鱼表情` 和「正文 + `<system_reminder>` / `<favour>`」两条 user 消息
  - 加载历史与落库时会去掉与相邻正文重复的名字前缀那条，只保留「正文 + 提示」形态
  - 旧会话下次对话加载时自动清洗，无需手清库
