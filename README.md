...
- 🔗 **与官方 AstrBot 完全兼容** – 配置、会话、人格、知识库、平台适配、插件 API 与官方一致，官方生态插件可直接安装使用  
- 💾 **数据备份与恢复** – WebUI 支持备份/恢复；可恢复不同版本备份数据，迁移时保留 `data/` 即可带走配置、会话与插件数据  
- 🧩 **插件安装与使用** – WebUI 插件管理 / 市场安装，或 `/plugin get <仓库地址>`；启用、禁用、重启、更新流程与官方一致  
- 📄 **文档入口** – 仓库 README / CHANGELOG，或 WebUI → 插件管理 → 内置插件 **ldm** 查看说明文档与更新文档  
...
- 🛑 **打断回复 + 内置分段** – 更接近真人聊天节奏，可与群聊上下文 / 主动回复配合使用  
